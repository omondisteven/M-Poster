...
function PosterPage() {
  ...
  useEffect(() => {
    const updatePreviewQr = async () => {
      ...
      if (qrGenerationMethod === "mpesa") {
        ...
        setPreviewQrData(`https://ebiz-payments-page-new-cwmj.vercel.app/?data=${encodedData}`);
      }
    };
      updatePreviewQr();
  ...
  const generateDownloadQrData = async (): Promise<string> => {
    ...
    try {
      ...
      // 4. Create the URL
      const originalUrl = `https://ebiz-payments-page-new-cwmj.vercel.app/?data=${base64Encoded}`;
